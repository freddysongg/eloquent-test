...
class Chat(Base):
    ...
    title: Mapped[str] = mapped_column(
        String(255), nullable=False, default="New Chat", doc="Chat conversation title"
    )

    description: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, doc="Optional chat description or summary"
    )
    ...
    # Relationships
    user = relationship("User", back_populates="chats", lazy="select")
    ...
    def add_tag(self, tag: str) -> None:
        ...
        tags = self.chat_metadata.get("tags", [])
        if tag not in tags:
            tags.append(tag)
            self.chat_metadata["tags"] = tags
    ...
    def remove_tag(self, tag: str) -> None:
        ...
        """
        if self.chat_metadata and "tags" in self.chat_metadata:
            tags = self.chat_metadata["tags"]
            if tag in tags:
                tags.remove(tag)
                self.chat_metadata["tags"] = tags
    ...
    def get_tags(self) -> list[str]:
        ...
        Returns:
            List of tag strings
        """
        return self.chat_metadata.get("tags", []) if self.chat_metadata else []

    def to_dict(
        self, exclude: Optional[set] = None, include_messages: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def __repr__(self) -> str:
        ...
        user_info = (
            ...
            if self.user_id
            else f"session_id={self.session_id}"
        )
