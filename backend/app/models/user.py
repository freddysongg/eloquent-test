...
class User(Base):
    ...
    display_name: Mapped[Optional[str]] = mapped_column(
        String(200), nullable=True, doc="User's preferred display name"
    )

    avatar_url: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, doc="URL to user's profile avatar"
    )
    ...
    preferences: Mapped[Dict[str, Any]] = mapped_column(
        JSON, default=dict, nullable=False, doc="User preferences and settings"
    )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"<User(id={self.id}, clerk_id={self.clerk_user_id}, email={self.email})>"
        )
